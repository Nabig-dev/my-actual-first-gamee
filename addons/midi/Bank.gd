...
class TempSoundFontInstrumentBagADSR:
	...
	var sustain_vol_env_db: float = 0.0
	var release_vol_env_time: float = 0.001
			...
			for gen_count in range(gen_index, gen_next):
				...
				match gen.gen_oper:
					...
					SoundFont.gen_oper_key_range:
						bag.key_range = TempSoundFontRange.new(gen.uamount & 255, gen.uamount >> 8)
			...
			for gen_count in range(gen_index, gen_next):
				...
				match gen.gen_oper:
					SoundFont.gen_oper_key_range:
						...
						bag.key_range.low = gen.uamount & 255
					SoundFont.gen_oper_vel_range:
						bag.vel_range.high = gen.uamount >> 8
						bag.vel_range.low = gen.uamount & 255
