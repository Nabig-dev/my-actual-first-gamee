...
class IconReplacer:
	...
	const PE_HEADER_ADDRESS_OFFSET: = 60
	const NUMBER_OF_SECTIONS_OFFSET: = 6
	const SIZE_OF_OPTIONAL_HEADER_OFFSET: = 20
	const MAGIC_OFFSET: = 24
	const COFF_HEADER_SIZE: = 24
	const SECTION_SIZE: = 40
	const SIZE_OF_RAW_DATA_OFFSET: = 16
	const POINTER_TO_RAW_DATA_OFFSET: = 20
	...
	func parse_entry(resources: PoolByteArray, offset: int, data_entries: Array) -> void :
		...
		if entry_offset & 2147483648:
			parse_table(resources, entry_offset & 2147483647, data_entries)
		else:
