...
class IconCreator:
	...
	const IHDR_SIGNATURE: = PoolByteArray([73, 72, 68, 82])
	const IDAT_SIGNATURE: = PoolByteArray([73, 68, 65, 84])
	const IEND_SIGNATURE: = PoolByteArray([73, 69, 78, 68])
	...
	func generate_icon_entry(image: Image, size: int, offset: int) -> PoolByteArray:
		var result: = PoolByteArray()
		result.append(image.get_width())
		result.append(image.get_height())
		result.append(0)
		result.append(0)
		result.append_array(lsb_first(0, 2))
		result.append_array(lsb_first(32, 2))
		result.append_array(lsb_first(size))
	...
	func generate_data_chunk(image: Image) -> PoolByteArray:
		...
		for i in range(zlib_block_count):
			...
			result.append(1 if last_block else 0)
	...
	func msb_first(i: int) -> PoolByteArray:
		...
		result.append((i >> 16) & 255)
		result.append((i >> 8) & 255)
		result.append(i & 255)
		return result
	...
	func block_size(i: int) -> PoolByteArray:
		...
		result.append(i & 255)
		result.append((i >> 8) & 255)
		result.append((i & 255) ^ 255)
		result.append(((i >> 8) & 255) ^ 255)
		return result
