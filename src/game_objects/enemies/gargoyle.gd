...
func _flap_snd() -> void :
	Audio.play_sfx("wing_flap")

func _on_Area2DDetectPlayer_area_entered(_area: Area2D) -> void :
	if Enemy.state == "sleep" and active == true:
		randomize()
		$TimerAwake.start(rand_range(0.5, 2))
		$Area2DDetectPlayer / CollisionShape2D.set_deferred("disabled", true)

func _on_TimerAwake_timeout() -> void :
