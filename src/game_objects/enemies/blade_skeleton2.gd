...
var Slash = preload("res://src/game_objects/enemies_weapons/annette_spark_projectile.tscn")
...
func return_to_patrol() -> void :
	if AreaPlayerFront.is_colliding() == false:
		...
		and Enemy.get_player_distance() < 50
	):
		Enemy.state = "idle"
		make_attack()
	else:
...
func _on_TimerChangePatrolState_timeout() -> void :
	...
	if (
		...
		and Enemy.get_player_distance() < 80
	):
		make_attack()
		return
