...
func change_state(
	...
	if new_state != state or force_change == true:
		...
		match state:
			...
			"atk-a", "atk-b", "atk-c", "atk-d", "atk-spin", "atk-h", "atk-charge", "atk-m":
...
func whip_attack(wtype: String = "atk-a") -> void :
	...
	if (
		...
		and anim_current.begins_with("atk") == false
	):
		...
		if (
			...
			change_state("atk-charge", false, false)
		...
		if anim_current == "atk-a" and percentage_timeline >= 35:
			_melee_whip = true
			
			return
		elif anim_current == "atk-b" and percentage_timeline >= 20:
			_melee_whip = true
			
			return
		elif anim_current == "atk-c" and percentage_timeline >= 20:
			_melee_whip = true
			
			return
		elif anim_current == "atk-d":
			_melee_whip = true
...
func _get_input() -> void :
	...
	if anim_current == "atk-charge" and Input.is_action_just_released("attack"):
		if _charge_status == "charged-blood":
			change_state("atk-m", true, false)
		elif _charge_status == "charged":
			change_state("atk-d", true, false)
		else:
			change_state("atk-a", true, false)
		_charge_status = ""
		AnimAtkCharge.play("RESET")
		return
...
func _fix_state() -> void :
	...
	if state in [
		"dead", "sit", 
		"mark", 
		"suffocation", "death-floor", "depress", 
...
func _player_touched_floor():
	if _character_ready and is_on_floor() and not _was_on_floor:
		...
		elif anim_current in ["atk-air"]:
			anim_state_machine.travel("atk-a")
...
func _on_attack_anim_finished(next_anim: String = "") -> void :
	...
	if is_on_floor():
		if (
			(Input.is_action_pressed("ui_down") or _player_can_stand() == false)
			and anim_current in ["atk-crouch", "pos-crouch"]
		):
			state = "crouch"
		else:
			change_state("idle")
