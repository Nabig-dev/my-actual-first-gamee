...
func _ready() -> void :
	...
	if VarsGlobal.has_flag("xandria_with_antagonist_1stdialog") == false:
		...
		VarsGlobal.GameScenario.get_node("Enemies/Johannes").queue_free()
		VarsGlobal.GameScenario.get_node("Enemies/Isaac").queue_free()
		VarsGlobal.GameScenario.get_node("Enemies/Eva").queue_free()
...
func _on_camera_tweened_to_position() -> void :
	...
	VarsGlobal.GameInterface.start_dialog("grijayla-main-enemies-dialog-11")
...
func _on_dialog_ended(dialog: String) -> void :
	if dialog == "grijayla-main-enemies-dialog-11":
		...
		VarsGlobal.GameScenario.get_node("Enemies/AnimationPlayer").play("atk")
