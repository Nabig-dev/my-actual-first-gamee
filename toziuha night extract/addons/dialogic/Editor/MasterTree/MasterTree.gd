...
func build_flat_tree_items(current_tree: String = ""):
	...
	if not current_tree in editor_reference.flat_structure:
		return
	...
	for entry in editor_reference.flat_structure[current_tree]:
		...
		while not (depth_stack[ - 1]["path"] in entry["key"]) and depth_stack.size() > 1:
...
func get_item_path(item: TreeItem) -> Dictionary:
	...
	if not "path" in item.get_metadata(0):
		return {"path": "/", "step": "0"}
