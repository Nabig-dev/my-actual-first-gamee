...
const drum_track_channel: int = 9
...
func _analyse_smf() -> void :
	...
	self.loop_start = 0.0
	self._used_program_numbers = [0, Bank.drum_track_bank << 7]
				...
				match event.number:
					SMF.control_number_bank_select_msb:
						if channel.number == drum_track_channel:
							...
							channel.bank = (channel.bank & 127) | (event.value << 7)
					SMF.control_number_bank_select_lsb:
						if channel.number == drum_track_channel:
							channel.bank = Bank.drum_track_bank
						else:
							channel.bank = (channel.bank & 16256) | (event.value & 127)
		...
		SMF.control_number_bank_select_msb:
			if channel.drum_track:
				...
				if value == 1:
					...
					value = 0
				channel.bank = (channel.bank & 127) | (value << 7)
		SMF.control_number_bank_select_lsb:
			if channel.drum_track:
				channel.bank = Bank.drum_track_bank
			else:
				channel.bank = (channel.bank & 16256) | (value & 127)
