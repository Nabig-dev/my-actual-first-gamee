...
func _on_Area2D_area_entered(_area: Area2D) -> void :
	...
	var john: KinematicBody2D = VarsGlobal.GameScenario.get_node("EventJohannes/Johannes1")
	
	john.Enemy.change_state("prism")
	...
	john.Enemy.change_state("prepare")
	yield(john.get_node("AnimationPlayer"), "animation_finished")
	...
	VarsGlobal.GameScenario.get_node("CanvasLayer/Control/BatleProgressBar").max_value = john.max_hits
	john.Enemy.change_state("idle2")
	john.start_battle()
...
func _on_finish_battle() -> void :
	...
	john.global_position = VarsGlobal.GameScenario.get_node("EventJohannes/PositionJohannesEvA").global_position
	john.Enemy.change_direction("-1")
	john.Enemy.change_state("finisher")
	...
	yield(get_tree().create_timer(0.5), "timeout")
	john.Enemy.change_state("finisher_interrupt")
	...
	john.velocity.x = 0
	john.Enemy.change_state("shield")
	john.get_node("TimerToUltraMegido").start()
	VarsGlobal.GameInterface.can_pause = true
	VarsGlobal.Player.set_enabled_input(true)
