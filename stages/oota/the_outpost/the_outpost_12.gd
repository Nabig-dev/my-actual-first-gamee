...
func _on_Area2DDetectPlayerStartFlashback_area_entered(_area: Area2D) -> void :
	...
	VarsGlobal.GameInterface.start_dialog("the_outpost-flashback-before-atreu")
	VarsGlobal.GameScenario.get_node("AtreuCinematic").visible = true

func _on_dialog_ended(dialog: String) -> void :
	
	match dialog:
		"the_outpost-flashback-before-atreu":
			...
			VarsGlobal.GameInterface.start_dialog("the_outpost-prologue-atreu-go")
		
		"the_outpost-prologue-atreu-go":
...
func _on_camera_tweened_to_player() -> void :
	...
	yield(get_tree().create_timer(1), "timeout")
	VarsGlobal.GameInterface.start_dialog("the_outpost-after-atreu")
