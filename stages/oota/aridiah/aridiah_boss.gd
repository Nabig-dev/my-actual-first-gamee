...
func _ready() -> void :
	...
	if VarsGlobal.has_flag("defeated_boss_elisia") == false:
		Audio.stop_music()
		VarsGlobal.GameScenario.get_node("Elisia").HurtboxEnemy.connect(
		...
		VarsGlobal.Player.stop_move()
		yield(get_tree().create_timer(2), "timeout")
		VarsGlobal.GameInterface.start_dialog("elisia-prebattle")
		yield(VarsGlobal.GameInterface, "dialog_ended")
		VarsGlobal.GameScenario.get_node("Elisia").start_battle()
		yield(get_tree().create_timer(2), "timeout")
...
func _on_defeat() -> void :
	...
	VarsGlobal.GameInterface.start_dialog("elisia-defeated")
	yield(VarsGlobal.GameInterface, "dialog_ended")
	
	VarsGlobal.GameInterface.can_pause = true
	VarsGlobal.Player.set_enabled_input(true)
	
	VarsGlobal.GameScenario.get_node("Elisia").AnimPlayer.play("defeated")
