...
var BossBat = preload("res://src/game_objects/bosses/the_screecher.tscn")
...
func _ready() -> void :
	...
	VarsGlobal.GameInterface.start_dialog("shay-in-train")
	yield(VarsGlobal.GameInterface, "dialog_ended")

	
	VarsGlobal.GameScenario.get_node("shay").Enemy.change_state("invoke")
	...
	VarsGlobal.GameScenario.get_node("shay").queue_free()
...
func _on_boss_orb_obtained() -> void :
	...
	if VarsGlobal.game_data["player_hp_now"] > 0:
		...
		SceneChanger.change_scene("res://stages/oota/the_barrens/aridiah_station.tscn")
