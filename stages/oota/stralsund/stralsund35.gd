...
func _on_AnimationPlayer_animation_finished(_anim_name: String) -> void :
	...
	VarsGlobal.GameScenario.get_node("Witiko").global_position = VarsGlobal.GameScenario.get_node(
		"BossAppearCinematic/ElementalCircuit"
	).global_position
	VarsGlobal.GameScenario.get_node("Witiko").gravity = 250
	
	VarsGlobal.GameScenario.get_node("Witiko/EnemyBase").change_direction("to_player")
	
	VarsGlobal.GameScenario.get_node("BossAppearCinematic").queue_free()
	
	yield(get_tree().create_timer(1), "timeout")
	
	VarsGlobal.GameScenario.get_node("Witiko/EnemyBase").connect(
...
func _on_TimerSpawnAzche_timeout() -> void :
	...
	ObjInstance.global_position = VarsGlobal.GameScenario.get_node("Witiko").global_position - Vector2(0, 170)
