...
func _ready() -> void :
	...
	if VarsGlobal.GameScenario.get_node("vial").BossNode.was_defeated():
		...
		if (
			...
			> VarsGlobal.GameScenario.get_node("vial").global_position.x
		):
			VarsGlobal.GameScenario.get_node("vial").global_position.x -= 300
...
func _on_AreaDetectPlayer_area_entered(_area: Area2D) -> void :
	...
	VarsGlobal.GameScenario.CameraNode.move_to(
		VarsGlobal.GameScenario.get_node("vial").global_position, 
		3
	)
	yield(VarsGlobal.GameScenario.CameraNode, "tweened_to_position")
	
	VarsGlobal.GameScenario.get_node("vial").start_battle()
